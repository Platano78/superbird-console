...
export function primaryDemoHost(nowMs: number): FleetHost {
  ...
  return {
    ...
    age_s: 2,
    error: null,
    leaf: { name: 'chat', known_leaves: ['chat', 'prod', 'swarm', 'pair', 'leaf-deep', 'leaf-alt'], transition: null },
    seats: [
      {
        id: 'worker',
        label: 'WORKER',
    ...
    commands: {
      // LEAVES renders from this roster, never a hardcoded list. `leaf-alt` is
      // off the daily list, so it lands in READY-FOR-DUTY and picks up the
      // uncensored tone; `leaf-deep` renders as LEAF-DEEP.
      flip: ['chat', 'prod', 'swarm', 'pair', 'leaf-deep', 'leaf-alt'],
      summon: ['herald'],
      dismiss: true,
