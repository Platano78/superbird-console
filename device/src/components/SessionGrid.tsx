...
function ago(ts: number, nowMs: number) {
...
export function SessionGrid({ sessions, nowMs }: { sessions: SessionSummary[]; nowMs: number }) {
    ...
    <div className="flex h-full gap-3 overflow-x-auto p-4">
      {sessions.map((s) => {
        ...
        return (
          <div
            key={s.id}
            className={`relative flex h-full w-56 shrink-0 flex-col justify-between overflow-hidden rounded-xl border-2 p-4 transition-colors duration-500 ${STATE_COLOR[s.state]}`}
          >
