...
const TILE_DEFS = [
  ...
  // Display names are THEME, protocol ids are CONTRACT — dsv4f stays dsv4f on
  // the wire (profile.sh leaf, action id); only the label wears the character.
  { key: 'dsv4f', label: 'GALACTUS', actionId: 'mb.profile.dsv4f', icons: { active: 'icon_mb_dsv4f_active.png', inactive: 'icon_mb_dsv4f_inactive.png' } },
...
export function MbSlot({ info, reachable }: Props) {
  ...
  if (mb.switching) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center">
        <div className="text-xl font-semibold text-amber-400">SWITCHING → {target === 'dsv4f' ? 'GALACTUS' : target.toUpperCase()}</div>
  ...
  // Header wears the same display name as the tile (dsv4f renders GALACTUS).
  const profileLabel = currentProfile === 'dsv4f' ? 'GALACTUS' : currentProfile ? currentProfile.toUpperCase() : '--'
