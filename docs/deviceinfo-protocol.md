...
| Field | Notes |
|---|---|
| `id` | `chat`\|`prod`\|`pair`\|`swarm`\|`dsv4f`\|`q38`\|`q38h` — wire id; display name is a UI concern (`dsv4f` renders GALACTUS) |
| `active` | derived from the one fingerprint chain that also sets `profile` — never a second source of truth |
| `tier` | `"daily"` (chat/prod/pair/swarm/dsv4f) or `"ready-for-duty"` (q38/q38h — owner ruling 2026-08-15, must not present as peers of the dailies) |
| `flags` | `["uncensored"]` for `q38h`, `[]` otherwise |
| `seats` | which seats this leaf reseats: `["worker"]` (prod/swarm/q38/q38h), `["senior"]` (dsv4f), `["worker","senior"]` (chat, **and pair** — `profile.sh`: *"P-PAIR: Ornith-9B fast lane @:8081 + DSV4F senior @:8080"*) |

🔴 **Fingerprint ordering trap.** `q38h`'s model path
(`/models/qwen38-27b-heretic/Qwen3.8-27B-heretic-ara.i1-Q6_K.gguf`) contains BOTH of
`q38`'s substrings (`qwen38-27b` and `Qwen3.8-27B`). `mb.js` tests `heretic` before either
q38 substring; reordering silently reports the uncensored leaf as stock. ✅ Proved this
session: the heretic model-id string classifies as `q38h`, the stock string as `q38`
...
| Field | Notes |
|---|---|
| `id` | the action id in flight, e.g. `mb.profile.q38h` |
...
deliberate flip (`mb-slot_spec.md` amend log, 2026-08-13).
...
Both `q38`/`q38h` default to `reasoning_effort: xhigh` and can legitimately return zero
...
own env) with a real `mb.profile.q38h` id: response was `{"id":"mb.profile.q38h","dryRun":true}`,
...
| `mb.profile.dsv4f` | `cd ~ && ./profile.sh dsv4f` | 8080 | completion |
| `mb.profile.q38` | `cd ~ && ./profile.sh q38` | 8081 | completion |
| `mb.profile.q38h` | `cd ~ && ./profile.sh q38h` | 8081 | completion |
...
✅ **`mb.profile.q38h` fired for real during this session's gate testing** (the DRYRUN
...
(q38h is worker-only), `lastResult: {id:"mb.profile.q38h", ok:true, ms:17043}`. This is
