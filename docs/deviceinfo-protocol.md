...
| Field | Notes |
|---|---|
| `switching.id` | the action id in flight, e.g. `mb.profile.q38h` |
...
1. **`POST /action {"id": "mb.profile.q38h"}`** (no `confirm_token`) → does **not**
   ...
   ```json
   {
     ...
     "expires_in_s": 30,
     "would": { "id": "...", "target": "q38h", "cmd": "cd ~ && ./profile.sh q38h", "ports": [8081], "verifyType": "completion" }
   }
   ```
   The token is bound to this exact `id`, expires in ~30 s, and is single-use.
2. **`POST /action {"id": "mb.profile.q38h", "confirm_token": "<token from step 1>"}`**
...
- `{"id":"mb.profile.q38h"}` → returned a `confirm_token`, `would`, spawned nothing.
- `{"id":"mb.profile.q38h","confirm_token":"<wrong>"}` → `400`, `confirm_token mismatch`,
  spawned nothing.
- `{"id":"mb.profile.q38h","dry_run":true}` → echoed `would`, spawned nothing.
